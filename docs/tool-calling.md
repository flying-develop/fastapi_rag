...
**Ограничение (осознанное, задокументированное):** только один раунд tool calling — без рекурсии/многошаговых агентных циклов. Если финальный ответ модели сам содержит `tool_calls` (модель снова запросила инструмент), функция всё равно вернёт его как есть — но залогирует `WARN` ("tool calls in final response — nested tool calling not supported"), чтобы это не прошло незамеченным. Полноценные многошаговые циклы — второй план вехи «Диалог как граф LangGraph» (см. [Диалог как граф LangGraph](dialog-graph.md)), сейчас `invoke_with_tools()` вызывается без изменений изнутри узла `agent` графа.
...
- [Диалог как граф LangGraph](dialog-graph.md) — граф, из узла которого сейчас вызывается `invoke_with_tools()`
